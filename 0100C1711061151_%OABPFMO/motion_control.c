...
void get_action_params(T_mtn_params* memd_params)
{
	switch(g_decision.action)
	{
		...
		case 4:
			...
			memd_params->memd_side = g_decision.params.onto_wire.enter_side;
			break;
		...
		default:
			rt_kprintf("[ljh]recv invalid action[0]!\n");
			break;
	}
	rt_kprintf("\n");
	return;
...
//				set_velocity(&motion.tracker, v, -omega);
//			else
//				set_velocity(&motion.tracker, v, omega);
//		}
//	}
	...
	float angle = acosf(dp)*57.3;
	//if(g_angle_cnt*omega*MTN_THREAD_TIME*57.3 >= 90)
	if(angle >= 175)
	{
		g_end_of_arc_reached = TRUE;
		g_dir_memd = FALSE;
