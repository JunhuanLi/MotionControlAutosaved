...
void action_params_print()
{
	switch(g_action)
	{
		...
		case 4:
			...
			break;
		default:
			rt_kprintf("Invalid action! No print!\n");
			break;
	}
...
void heading_control()
{
	...
	{
		case ONTO_WIRE:
			track_vector(&motion.tracker, g_action_params.onto_wire_.fin_vec[0], g_action_params.onto_wire_.fin_vec[1], ONTO_WIRE_LINEAR_VEL);
		break;
...
void onto_wire(T_gp_side enterSide, float rot_vel)
{
...
//			}
	...
	{
		onto_wire(g_action_params.onto_wire_.enter_side, ONTO_WIRE_ROT_VEL);
	}
		/*OBS*/
