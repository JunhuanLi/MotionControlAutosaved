...
T_bool DABUG_DIRLOCK_FLAG = FALSE;
T_bool DABUG_ONETIME_LOCK_FLAG = FALSE;
float dp=0.0;
float dp2=0.0;
/* macros *********************************************************************/
...
//			}
		...
		else//???¨²??¨¦?
		{
			...
			/*lijunhjuan 20171012 changed cuz ontowire*/
//			if((obj->sense.side_l == 1) && (obj->sense.side_r == 2))//¡¤¡ä¡¤??¨°,¦Ì?¨ª¡¤
//			{
//				obj->angular_vel = 0.3; 
//				//obj->angular_vel = 0;
//				obj->line_vel = 0;
...
//			
//			else 
			if(obj->sense.side_l != obj->sense.side_r)					//????¡Á?
			{
        ...
        {
          dp = g_action_params.onto_wire_.fin_vec[0]*obj->sense.dir_x + g_action_params.onto_wire_.fin_vec[1]*obj->sense.dir_y;
          dp2 = (-g_action_params.onto_wire_.fin_vec[0]*obj->sense.dir_x) + (-g_action_params.onto_wire_.fin_vec[1]*obj->sense.dir_y);
					if( (acosf(dp2)*57.3 <= 5)&& !DABUG_ONETIME_LOCK_FLAG)
					{
						DABUG_DIRLOCK_FLAG = TRUE;
						DABUG_ONETIME_LOCK_FLAG = TRUE;
					}
					
					if( (acosf(dp)*57.3 <= 5) && DABUG_ONETIME_LOCK_FLAG)
						 {
                //obj->path_mag_line.state = ZIGZAG_CONTINUE;
...
//							g_action = DIR_DRIVE;
//							g_action_params.dir_drive_.fin_vec[0] = g_action_params.onto_wire_.fin_vec[0];
//							g_action_params.dir_drive_.fin_vec[1] = g_action_params.onto_wire_.fin_vec[1];
                //rt_kprintf("MISSION COMPLETE! STOP\n");
            }
        }
