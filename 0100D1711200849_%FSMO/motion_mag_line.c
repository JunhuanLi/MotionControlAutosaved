...
const float Mag_Dir_Param_P = 0.02f; //0.02f
const float Mag_Dir_Param_I = 0.0f;  //
const float Mag_Dir_Param_D = 8.5f; //10.5f

const float tracking_high_P = 0.1;
const float tracking_high_D = 10.5;
const float tracking_low_P = 0.03;
const float tracking_low_D = 10.5;
...
#define WIRE_LINEAR_VELOCITY 0.2
...
{		
	...
	{
		if(obj->path_mag_line.track_dir == MOTION_WIRE_CCW)
		{
			if(obj->sense.side_l == MOTION_MAG_LINE_INSIDE)
			{
...
//		}
	...
	else
	{
		if(obj->sense.side_l != obj->sense.side_r)
		{
			Mag_Dir_err = (fabs(obj->sense.value_l)-1) - (fabs(obj->sense.value_r)-1);
			...
			{
				Mag_Dir_P = Mag_Dir_Param_P * Mag_Dir_err;
				//Mag_Dir_I = obj->path_mag_line.mag_tracking_pi.ki;
				Mag_Dir_D = Mag_Dir_Param_D * (Mag_Dir_err - Mag_Dir_err_last);
				//I\D not in use
				Mag_Dir_err_last = Mag_Dir_err;
				if(1)
				{
					obj->angular_vel = -(Mag_Dir_P + Mag_Dir_I + Mag_Dir_D);
				}
				else
				{
					obj->angular_vel = (Mag_Dir_P + Mag_Dir_I + Mag_Dir_D);
			...
			obj->line_vel = WIRE_LINEAR_VELOCITY;
...
/*************************************************
  Function:       // mtn_wire_turn
  Description:    // turn to make the vehicle body tracking with correct direction
  Input:          // pointer of tracker and track direction
  Output:         // turn finished or not
  Others:         // none
*************************************************/
T_bool mtn_wire_turn(T_motion_tracker* obj, T_motion_wire_track_dir track_dir)
{
	T_bool temp_turn_finished = FALSE;
	if(obj->path_mag_line.track_dir == MOTION_WIRE_CCW)
	{
		if(obj->sense.side_l == obj->sense.side_r)
		{
		...
		{
			// onwire but w/ wrong direction
			if(obj->sense.side_l == MOTION_MAG_LINE_INSIDE)
			{
				obj->angular_vel = 0.3;							
				obj->line_vel = 0;	
			}
			else //right direction, go ahead
			{
				temp_turn_finished = TRUE;
			}
		}
	}
	
	else if(obj->path_mag_line.track_dir == MOTION_WIRE_CW)
	{
		if(obj->sense.side_l == obj->sense.side_r)
		{
			if(obj->sense.side_l == MOTION_MAG_LINE_INSIDE)
	...
	else
	{
		...
		rt_kprintf("Invalid wiretracking state[0].\n");
		#endif
	}
	return temp_turn_finished;
}

/*************************************************
  Function:       // mtn_wire_tracking
  Description:    // tracking the wire
  Input:          // pointer of tracker
  Output:         // None
  Others:         // none
*************************************************/
static void mtn_wire_tracking(T_motion_tracker* obj)
{
	Mag_Dir_err = fabs(obj->sense.value_l) - fabs(obj->sense.value_r);
	
	if(abs(Mag_Dir_err)<=10)
	{} 
	else if(Mag_Dir_err>=100)
	{
		Mag_Dir_P = tracking_high_P * Mag_Dir_err;
		Mag_Dir_D = tracking_high_D * (Mag_Dir_err - Mag_Dir_err_last);
	}
	else
	{
		Mag_Dir_P = tracking_low_P * Mag_Dir_err;
		Mag_Dir_D = tracking_low_D * (Mag_Dir_err - Mag_Dir_err_last);
	}
...
/*************************************************
  Function:       // mtn_wire_turn_n_track
  Description:    // turn to the right direction and begin to track
  Input:          // pointer of tracker
  Output:         // None
  Others:         // none
*************************************************/
T_bool tmp_bool = FALSE;
void mtn_wire_turn_n_track(T_motion_tracker* obj)
{
	float dp_with_exit_dir = 0;
	float dp_with_enter_dir = 0;

	if(mtn_wire_turn(obj, motion.tracker.path_mag_line.track_dir))
		mtn_wire_tracking(obj);
		
	
  if( (g_decision.action == ONTO_WIRE) && (g_decision.params.onto_wire.laps == 1) )
  {
		dp_with_exit_dir = g_decision.params.onto_wire.fin_vec[0]*obj->sense.dir_x + g_decision.params.onto_wire.fin_vec[1]*obj->sense.dir_y;
    dp_with_enter_dir = (-g_decision.params.onto_wire.fin_vec[0]*obj->sense.dir_x) + (-g_decision.params.onto_wire.fin_vec[1]*obj->sense.dir_y);
	...
	else if (g_decision.params.onto_wire.laps == 0)
	{
	}
  else
	{
		#ifdef JHL_SERIAL_DEBUG
		rt_kprintf("Error when tracking wire bc action is not ONTO_WIRE w/ laps=1 nor w/ laps=0.\n \n");
		#endif
	}		
}
...
static float Motion_Mag_Line(T_motion_tracker* obj)
{
	...
	{	
		...
		else if(obj->path_mag_line.track_dir == MOTION_WIRE_CCW)
		{
			if(obj->sense.side_l == MOTION_MAG_LINE_OUTSIDE)
			{
				rot_dir = MOTION_TURN_COUNTERCLOCKWISE;
...
void Motion_Start_Mag_Line(T_motion_tracker* obj,float vel,T_motion_wire_track_dir dir)
{
	obj->tracking										= MOTION_TRACKING_MAG_LINE;
	obj->path_mag_line.state	 			= MOTION_MAG_LINE_STATE_START;
	obj->command_vel								= vel;
	obj->target_vel									= vel;
	obj->path_mag_line.track_dir	 				= dir;
