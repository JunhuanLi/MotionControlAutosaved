...
void Motion_Set_cornerAngle_Param(T_motion_tracker* obj,float kp, float ki, float il)
{
	...
	obj->path_imu.pre_dir_stored = FALSE;
}
...
*/
void rotate_angle(T_motion_tracker* obj, float angle, T_gp_side rot_dir) //angle is in degree
{
	//float											pi_out=0.0;
	...
	if(obj->path_imu.pre_dir_stored == FALSE)
	{
		obj->path_imu.ra_pre_dir_x = obj->sense.dir_x;
		obj->path_imu.ra_pre_dir_y = obj->sense.dir_y;
		obj->path_imu.pre_dir_stored = TRUE;
	}
	
	dot_product = (obj->sense.dir_x * obj->path_imu.ra_pre_dir_x)+(obj->sense.dir_y * obj->path_imu.ra_pre_dir_y);
...
void rotate_vector(T_motion_tracker* obj, float vec_x, float vec_y, T_gp_side rot_dir, float linear_vel) //angle is in degree
{
	//转角度测试相关参数
	...
	dot_product = (obj->sense.dir_x*vec_x)+(obj->sense.dir_y*vec_y);
	...
	if(dot_product < -1.0)
 	...
 	angleError = acosf(dot_product)*57.3;
	
	if(fabsf(angleError)<=10) //in degree
	{
		obj->path_imu.cornerAngle_pi.integral = 0;
		obj->path_imu.rotationFinished = TRUE;
		//rt_kprintf("rot finished.\n");
	}
	else
	{
		pi_out = PI_Run2(&(obj->path_imu.cornerAngle_pi),-angleError);

		if(rot_dir == RIGHT_STEERING)
		{
...
void track_vector(T_motion_tracker* obj, float vec_x, float vec_y, float linear_vel)
{
	...
	//Motion_Norm_2D(&vec_x,&vec_y);
	dp = vec_x*obj->sense.dir_x + vec_y*obj->sense.dir_y;
...
void track_point(T_motion_tracker* obj, float target_x, float target_y)
{
	...
	if(dist < 0.2)//m
	{
		//rt_kprintf("dist=%d:",(int)(dist*1000));
		obj->path_imu.pointReached = TRUE;
		//rt_kprintf("%d th point reached! \n", cnt111);
	}
	else
	{
		Motion_Norm_2D(&vec_x,&vec_y);
...
/*************************************************
  ...
  Description:    // get the turn direction by getting the cross product of two vectors
  Input:          // NONE
  Output:         // none
  Others:         // none
*************************************************/
...
{
	float cp = 0;
	cp = motion.tracker.sense.dir_x * tgt_dir_y - motion.tracker.sense.dir_y * tgt_dir_x;
	return cp<0 ? LEFT_STEERING : RIGHT_STEERING;
}

/*************************************************
  Function:       // to_pose_r2tdir
  Description:    // rotate to the target direction
  Input:          // NONE
  Output:         // none
  Others:         // none
*************************************************/
T_bool to_pose_r2tdir()
{
	static float vx;
	static float vy;
	static T_gp_side rd;
	
	if(g_tp_r2tdir_stage == 0)
	{
		vx = g_decision.params.to_pose.pos[0] - motion.tracker.sense.pos_x;
		vy = g_decision.params.to_pose.pos[1] - motion.tracker.sense.pos_y;
		rd = get_turn_dir(vx, vy);
		g_tp_r2tdir_stage = 1;
		rt_kprintf("rd: %d\n", rd);
	}
	
	if(g_tp_r2tdir_stage == 1)
	{
		if(!motion.tracker.path_imu.rotationFinished)
		{
			rotate_vector(&motion.tracker, vx, vy, rd, 0);
			return FALSE;
		}
		else
		{
...
/*************************************************
  ...
  Description:    // set velocity at the end of this thread
  Input:          // NONE
  Output:         // none
  Others:         // none
*************************************************/
void set_velocity(float linear_vel, float angular_vel)
{
	...
	motion.tracker.angular_vel = angular_vel;
}

/*************************************************
  Function:       // set_velocity_hard
  Description:    // set velocity instant
  Input:          // NONE
  Output:         // none
  Others:         // none
*************************************************/
